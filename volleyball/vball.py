...
url_names = 'https://www.maxpreps.com/rankings/volleyball-fall-17/{}/national.htm'
url_scores = 'https://www.maxpreps.com/high-schools/{})/volleyball-fall-17/schedule.htm'
...
for name in tqdm(my_keys):
    ...
    try:
        team_1=(soup.find('h1', attrs={'class':'Heading__StyledHeading-sc-1ape2tl-0 enjvvZ'}).text)
        team_1 = team_1[0:team_1.find("Volley")-1]
    ...
    for item in soup.find_all('tr'):
        ...
        # address=('nan,nan,nan,nan')
        try:
            score=(item.find('span', attrs={'class':'cwcKEO'}).text)
        except:
            score=('nan-nan')
        try:
            # here is wherer we need to strip the href out of the tag to get away team city
            element = item.find('a', attrs={'class':'hCJxpY'})
            team_2=(element.text)
        ...
        try:
            # print(item)
            result=(item.find('span', attrs={'class':'cMUoOp'}).text) # win stylization
        except: # the loss stylization is of class type sc-eCssSg  dyMwZa. if they don't have win, it's a loss
            try:
                result=(item.find('span', attrs={'class':'dyMwZA'}).text) # loss stylization
            except:
                try:
                    result=(item.find('span', attrs={'class':'iFDdfx'}).text) # actual tie stylization
                except:
                    result=("NoScoreReported")
        try:
            date_of_contest=(item.find('div', attrs={'class':'dbwfVZ'}).text)
        except:
            date_of_contest=(np.nan)
        try:
            place_played=(item.find('div', attrs={'class':'lkbXfm'}).text)
        except:
            place_played=(np.nan)
        try:
            time_of_contest=(item.find('div', attrs={'class':'ghwZQw'}).text)
        except:
