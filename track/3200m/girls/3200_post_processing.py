...
# notify.send('Running')
df = pd.read_csv('~/Documents/texas_football/track/3200m/girls/girls_3200m_1_28.csv', names=['School', 'Rank', 'Time_String',  'Date', 'Address'])
# df = pd.read_csv('~/Documents/texas_football/field/long_jump/boys/boys_longjump_header.csv', names=['School', 'Rank', 'Distance_String', 'Wind', 'Date', 'Address'])
...
df = df.drop(columns={'Address',3,4})
...
for index,row in time_pair.iterrows():
    seconds = float(row[1])
    seconds = seconds + float(row[0])*60
    time_numbers.append(seconds)

# dist_numbers_series = pd.Series(dist_numbers)
time_numbers_series = pd.Series(time_numbers)
df = pd.concat([df, time_numbers_series], axis=1)
df = df.rename(columns={0:'Time_Seconds'})

for index,row in df.iterrows():
    row['Rank'] = int(row['Rank'])-4

df = df[['School', 'Rank', 'Time_String', 'Time_Seconds',  'Date', 'Street_Address', 'City', 'State', 'Zip']]
...
df.to_csv('clean_girls_3200m_1.csv')
